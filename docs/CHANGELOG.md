...
用户手动调整了 `README.md` 的源码格式：每个 `##` 分区标题前统一改成
3 个空行；截图展示区四段说明和部署提示的 blockquote 改成整段单行
（不影响渲染，纯源码可读性调整）。逐项对照中文版现状，把同样的格式
同步进了英文/日文版。

**验证**（REQ-097）：`awk` 统计过三份文件共 27 处 `##` 标题前都精确
是 3 个空行。`grep` 核对过链接/图片引用数量不变。真实渲染效果需要
用户确认（预期不受影响）。
...
用户反馈截图展示区说明还是太单薄。用 `Read` 工具实际打开四张截图
逐一确认真实显示内容后重写文案：日历页补了"周期事件一次配置、自动
重复出现"这个机制说明；计划页补了"从计划直接把关键事件加进日历新建
事件"的实际使用体验；奖励页补了"任意心愿都行"的措辞和"实时显示还差
多少点"这个细节；设置页把外观自定义（字体/强调色/背景图本地上传）
和地区说明（仅影响节假日标注）分开点出来，不再笼统带过。三语言同步
翻译，信息量对等。

**验证**（REQ-096）：纯文档改动。用 `Read` 工具核对过文案里每个具体
细节都真实存在于对应截图，不是编的。真实渲染效果需要用户自己确认。
