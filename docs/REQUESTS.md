...
描述：
用户要求把系统默认配置改一下：默认主题从深色改成浅色；点数设置的
默认值从平日/周末都是 1000，改成平日 100、周末 200。两处都在
`createDefaultState()` 里，只影响**新安装**（`fillMissingDefaults()`
是"老数据里已经有这个字段就不覆盖"的浅展开合并逻辑，见文件里
`settings: {...defaults.settings, ...parsed.settings}` 这行），已经
有数据、已经调整过这两个设置的老用户不受影响，不需要任何迁移逻辑。

验收：
- 全新安装（或者清空数据库重新打开）默认是浅色主题
- 全新安装的点数换算，平日是 100 点/小时，周末是 200 点/小时（设置页
  能看到，也能在这里手动改）
- 已有数据、之前手动调整过主题/点数设置的用户，这次改动不影响他们
  已经保存的设置值

验证：
`node --check` 通过。手工核对了 `fillMissingDefaults()` 的合并逻辑，
确认这两个字段走的是"老数据优先，缺失才用新默认值"这条已有路径，
不会覆盖老用户的设置。真实浏览器里全新安装的默认外观/点数显示效果
需要用户本地验证。
...
描述：
用户要求"根据用户的系统配置自动配置语言，然后设置地区"——新安装
（`createDefaultState()`）的 `settings.locale`/`settings.region` 从
一直写死 `'zh'`/`'CN'`，改成读浏览器的 `navigator.languages`/
`navigator.language` 自动判断。跟 REQ-092 的主题/点数默认值是同一类
改动：只影响**全新安装**，`fillMissingDefaults()` 走"老数据里已经有
这个字段就不覆盖"的浅展开合并（见文件顶部注释），已经打开过应用、
已经有自己设置的用户完全不受影响，不需要任何迁移。

新增两个内部函数：
- `detectDefaultLocale()`：遍历 `navigator.languages`（优先，是一个
  按用户系统偏好排序的列表）或退化到单个 `navigator.language`，取
  第一个能匹配上 `zh`/`ja`/`en`（这三个语言前缀，忽略地区子标签，比如
  `zh-TW`/`zh-HK` 都会命中 `zh`）的语言，都不匹配（比如系统语言是
  法语/德语这种应用完全不支持的语言）或者没有 `navigator`（比如测试
  环境）时兜底成 `'en'`——用户明确要求识别不出来时默认英文/美国，不是
  中文/中国大陆。
- `detectDefaultRegion(locale)`：优先直接从语言标签的地区子标签读
  （比如 `zh-CN` 里的 `CN`），命中 `domain/holidays.js`
  `SUPPORTED_REGIONS`（`CN`/`US`/`JP`，两处地方列的是同一份，注释里
  写明了要同步改）这三个之一就直接用；读不到或者不在支持范围内
  （比如 `en-GB`、`zh-TW`），按上一步识别出的语言给一个合理的默认
  地区（`zh→CN`、`en→US`、`ja→JP`）——不追求对每个地区子标签都精确
  覆盖，只是"猜一个大概率对的起点"，用户自己在设置页随时可以改，这个
  取舍在代码注释里写明了。

验收：
- 全新安装（或清空数据库重新打开），浏览器语言是简体中文相关
  （`zh`/`zh-CN`/`zh-TW` 等）时，应用默认显示中文、地区默认中国大陆
- 浏览器语言是英文（`en-US`/`en-GB` 等）时，默认显示英文、地区默认
  美国
- 浏览器语言是日文（`ja`/`ja-JP`）时，默认显示日文、地区默认日本
- 浏览器语言是这三种都不沾边的语言（比如法语），或者识别不出浏览器
  语言时，默认英文 + 美国
- 已有数据的老用户，这次改动完全不影响他们已经保存的语言/地区设置

验证：
`node --check` 通过。写了一份独立的模拟测试脚本（在 Node 里用
`globalThis.navigator` 造出不同的 `languages`/`language` 组合），
覆盖了 `zh-CN`/`en-US`/`ja-JP`/`en-GB`/`zh-TW`/完全不支持的语言
（法语+德语）/`languages` 为空数组退化到单个 `language`/完全没有
`navigator`（模拟测试环境） 这几种场景，逐一核对了实际返回的
`{locale, region}` 组合跟预期一致，包括几个边界情况的兜底行为
（`en-GB` 拿到 `US`、`zh-TW` 拿到 `CN`、完全不支持的语言和没有
`navigator` 都正确退回 `en`/`US`）。真实浏览器（不同操作系统/浏览器
语言设置）下的实际检测效果需要用户本地验证。
...
描述：
用户自己手动编辑了 `README.md`（中文版），加了一段"页面样式效果"
截图展示区（日历/计划/奖励/设置四张页面截图各配一段说明），改了标语
和副标题措辞，要求把这些改动同步到英文/日文版，并且指定英文标语用
"Plant your plans. Let time grow them."。

同步过程中顺手修正了一个技术性问题：用户加的图片路径写的是
`img\image-....png`（反斜杠），这在 GitHub 上不会正常渲染——markdown
里的相对路径必须用正斜杠，反斜杠只是在某些本地 Windows 环境的预览器
里凑巧能显示。三份文件（含中文原文）里全部四张图片的引用都统一改成了
`img/image-....png`。

三语言标语翻译：
- 中：`种下计划,让时间管理它的成长。`（用户改的）
- 英：`Plant your plans. Let time grow them.`（用户指定）
- 日：`計画を植えよう。育てるのは時間。`（跟英文版结构对应的意译，
  不是逐字直译）

副标题（"写给...的人的...工具"这句）中文加了"计划执行日历管理"这个
修饰，英文/日文版也相应改成强调"日历型"这个定位，不是简单沿用原来的
"execution tool"/"実行支援ツール"这种更笼统的措辞。截图展示区的四段
说明文字按中文原意翻译成英文/日文，图片引用保持相同的四个文件名，
"可以通过下面链接试用"这句故意没有补一个实际链接——GitHub Pages 部署
当时还在联调，没有把握确认它稳定可用，插入一个可能失效的链接比留着
文字描述更糟，这个留给用户自己决定什么时候补上、补哪个链接。

验收：
- 三份 README 打开都能看到标语、副标题、截图展示区，内容对应一致
  （不是逐字机翻，但传达的信息点一一对应）
- 四张截图在 GitHub 网页上（不是本地 Windows 文件管理器/编辑器预览）
  能正常显示，不会因为路径分隔符问题而 404

验证：
纯文档改动，没有可执行代码。用 `grep` 核对过三份文件里全部图片引用
都已经统一成正斜杠路径，`img/` 目录下四个文件名跟引用的文件名逐一
核对过存在且拼写一致。真实在 GitHub 网页上查看渲染效果（截图是否
正常显示、三语言切换链接是否都能跳转到对应版本）需要用户自己确认。
...
描述：
REQ-094 之后用户要求三件事：①之前故意留白的"通过链接试用"补上实际
地址；②中文原文里语气较随意、标点不统一的地方润色一下，三语言同步
润色成统一风格；③日语标语从这次改的"計画を植えよう。育てるのは
時間。"改回 REQ-084 时最初定的"計画が根を張り、一歩ずつ育っていく。"
（中文/英文标语保持这次改的新版本不变，只有日语的还原）。

补链接之前先用 `curl` 实际核对了 GitHub Pages 现在的部署状态——之前
几轮排查的"Deploy from a branch"配置错误、workflow 没触发这些问题
都已经解决，线上返回的是正确带 `/Planto/` hash 路径的构建产物，
`sqlite3.wasm`/`sqlite3-opfs-async-proxy.js` 都是 200，确认可以放心
把 `https://yiran201.github.io/Planto/` 写进 README，不是没验证过就
贴一个可能失效的链接上去。

润色范围：给"页面样式效果"这段加上 `## 📸` 级别的标题（跟全文其它
分区统一用"## + emoji + 标题"这个格式，原来这段是纯文字没有走
markdown 标题语法）；四段截图说明从口语化的流水账（"你可以...并随时...
能够很直观地..."这种句式）改写成跟其它分区一致的"**加粗小标题**：
一句话描述"的紧凑格式；图片的 alt 文字从原始文件名（`image-
20260907163058452`）改成有意义的页面名字（"日历页"/"计划页"等）；
副标题里"计划执行日历管理工具"这种堆砌修饰词的表述精简成"日历化计划
执行工具"；中文intro段落重新按原有惯例做了软换行（不影响渲染效果，
只是方便看源码）；统一了标语里全角/半角标点混用的问题（英文逗号改成
中文全角逗号）。这些润色逐条同步翻译进了英文/日文版，保持三语言风格
一致，不是各自随意发挥。

验收：
- 三份 README"截图展示"这个分区都有统一格式的标题、加粗小标题 + 一句话
  说明的段落结构
- 三份文件里"在线体验"这句都指向 `https://yiran201.github.io/Planto/`
  这个真实可访问的地址
- 中文/英文标语维持这次改的新版本，日语标语是"計画が根を張り、
  一歩ずつ育っていく。"（跟中文"种下计划，让时间管理它的成长。"字面
  上不再是逐字对应关系，是刻意保留的旧版本，不是翻译疏漏）

验证：
纯文档改动。`curl` 实测过 GitHub Pages 线上地址的 HTTP 状态码和
资源路径确认部署正常。`grep` 核对过三份文件里在线体验链接的地址
完全一致。真实网页排版效果（标题层级、加粗文字渲染、链接可点击）
需要用户自己在 GitHub 上确认。

---

## [REQ-096] 截图展示区内容丰富化——按实际截图内容重写四段说明

状态：completed
模块：README.md, README.en.md, README.ja.md

描述：
用户反馈截图展示区的四段说明还是不够丰富、格式想再贴近全文其它分区
的风格。排查发现问题不在格式本身（REQ-095 已经统一成"加粗小标题 +
描述 + 图片"的结构），而在内容太单薄——原来的说明只是转述"这是XX页"
这层意思，没有具体信息量，跟"✨ 核心功能"那几条信息密度高的介绍比
明显单薄。

用 `Read` 工具实际打开了 `img/` 目录下四张截图（而不是凭标题猜内容），
逐张看清楚了界面上真实显示的内容再重写文案，确保新增的细节都有截图
依据，不是编的：
- 日历页截图实际显示的是一周视图，里面有多条设成周期事件的日常习惯
  （起床洗漱、学英语、工作/学习、健身、料理、洗澡刷牙护肤这类），
  侧边栏有年度进度条，背景是一张自定义图片——文案据此加了"周期事件
  一次配置、自动重复出现，不用每周手动重排"这个具体机制说明
- 计划页截图显示多个真实计划（骑行计划、打工度假、大环线旅行、潜水
  旅行），每个都有"N / M 已完成"进度、关键事件的编辑/删除入口、
  "Cancel plan"按钮，还有一行提示"可以从计划直接把关键事件加进日历
  新建事件"——文案补全了"不用来回切页面对照"这个实际使用体验
- 奖励页截图显示当前点数余额、一份自定义奖励清单（数码产品/旅行/
  装备等），每项还差多少点数实时显示，够了的那一项显示可兑换——文案
  加了"从一顶球鞋到一趟旅行都行"这种具体到"任意心愿"的措辞，避免
  读者以为奖励类型有限制
- 设置页截图显示语言切换、外观（字体/强调色/背景图，含"浏览本地
  文件"入口）、地区（备注"仅影响节假日标注，不影响排程逻辑"）、
  点数换算比率（平日/周末分别设置）——文案把这几块分别点出来，之前
  只笼统提了"语言和地区""点数比例"两项，漏了外观自定义这部分

三段说明的加密程度参考了"✨ 核心功能"每条 bullet 的信息密度（不是
单纯堆字数），逐条同步翻译进了英文/日文版，保持三语言的信息量对等，
不是中文丰富、其它两个语言版本敷衍。

验收：
- 三份 README 的截图展示区四段说明都包含至少一个从截图里能验证到的
  具体细节（不是"你可以管理你的计划"这种空泛表述）
- 三语言版本传达的信息点一一对应，不存在某个语言版本明显更简略

验证：
纯文档改动。用 `Read` 工具实际查看了全部四张截图确认文案里提到的
每个具体细节（周期事件示例、计划页的"N/M 已完成"进度显示、奖励页的
"还差 X 点"实时提示、设置页的"浏览本地文件"按钮和地区说明文字）都
真实存在于对应截图里，不是杜撰。真实网页排版效果需要用户自己在
GitHub 上确认。

---

## [REQ-097] README 三语言版本同步分区间距 + 部分段落改成单行格式

状态：completed
模块：README.md, README.en.md, README.ja.md

描述：
用户自己手动调整了 `README.md`（中文版）的源码格式：每个 `##` 分区
标题前统一改成 3 个空行（原来是 1 个）；截图展示区四段说明、部署
提示的 blockquote 从原来软换行成多行的写法改成每段一整行；其余段落
（开头介绍段、"在线体验"、"文档"分区）保持原有的软换行不变。要求把
这套格式同步到英文/日文版。

这类改动纯粹是源码可读性层面的取舍（markdown 单个换行不产生段落
边界，渲染出来的 HTML 完全不受影响），不影响任何用户实际看到的页面
效果，逐项对照 `README.md` 现在的状态，在英文/日文版里做了同样位置、
同样处理方式的调整——不是自己发明一套新格式，是照着中文版这份"参考
文件"逐段落对应过去的。"📖 文档/Documentation/ドキュメント"这个分区
中文版当时的软换行断得比较随意（前半句和后半句在两行之间断得不是
词组边界），判断是编辑过程中的手误而非刻意格式，没有照抄这处的具体
断行位置，改成了跟其它"已改成单行"的段落一致的整段单行处理，保持
英文/日文版内部逻辑自洽。

验收：
- 三份 README 里每一个 `##` 分区标题前都是 3 个连续空行，位置一一
  对应
- 截图展示区四段说明和部署提示的 blockquote，三语言版本里都是没有
  软换行的整段单行
- 三份文件在 GitHub 网页上渲染出来的视觉效果跟改动前完全一致（这类
  改动本来就不影响渲染）

验证：
纯文档改动。写了一段 `awk` 脚本统计三份文件里每个 `##` 标题前的连续
空行数量，确认全部 9 个分区、三个文件共 27 处都是精确的 3 个空行，
没有遗漏或者多算/少算的情况。`grep` 核对过在线体验链接和四张截图的
引用路径在重新整理格式后依然完整、数量不变（各 1 处链接、4 处图片
引用）。真实网页渲染效果需要用户自己在 GitHub 上确认（预期不受这次
改动影响）。
